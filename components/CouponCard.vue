<template>
    ...
    >
      <div class="flex-1">
        <div class="text-2xl font-bold" :class="coupon.type === 'DISCOUNT' ? 'text-indigo-800' : 'text-green-800'">
          <span v-if="coupon.type === 'DISCOUNT'">
            NT$ {{ coupon.discountPrice }} 元折扣
          </span>
          <span v-else-if="coupon.type === 'PERCENTAGE'">
            {{ coupon.couponPercentage }}% 折扣
            <span v-if="coupon.maxPrice"> (最高折 NT$ {{ coupon.maxPrice }})</span>
          </span>
        </div>
      ...
      <div class="text-right flex flex-col items-end">
        <div class="text-lg font-semibold text-gray-700 bg-gray-50 px-3 py-1 border border-dashed rounded-md">
          {{ coupon.code }}
        </div>
        <p class="text-xs text-gray-500 mt-1 mb-3">優惠碼</p>

        <Button size="sm" variant="secondary" @click="handleCopy(coupon.code)">
          複製代碼
        </Button>
      </div>
    </Card>
